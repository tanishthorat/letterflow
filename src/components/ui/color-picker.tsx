...
export function ColorPicker({ value, onChange, className, align = "right" }: ColorPickerProps) {
  ...
  // Handle click outside to close color picker popover
  useEffect(() => {
  ...
  return (
    <div ref={containerRef} className={cn("relative w-full", className)}>
      ...
      <button
        ...
        className="flex items-center justify-between w-full h-10 px-3 py-2 bg-background border border-input rounded-md text-sm text-foreground hover:bg-muted/30 transition-all cursor-pointer select-none"
      >
      ...
      {isOpen && (
        <div className={cn(
          "absolute top-11 p-3 bg-popover border border-border rounded-lg shadow-xl z-50 animate-in fade-in slide-in-from-top-1 duration-150 w-64",
